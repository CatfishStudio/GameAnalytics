package gameAnalitics.testingAB 
{
	 ...
	 */
	public class TestingABDiagramUserID extends Sprite 
	{
		...
		public function TestingABDiagramUserID(userID:String) 
		{
			super();
		...
		{
			var sqlCommand:String = "SELECT * FROM ab_testing WHERE(ab_testing_userid = '" + _userID + "') ORDER BY ab_testing_datetime";
		...
		private function buildTable(event:Object):void
		{
			...
			_dataGrid.columns = ["data", "session", "record"];
			...
			_dataGrid.columns[2].headerText = "Запись";
		...
		{
			var sqlCommand:String = "SELECT ab_testing_userid, ab_testing_datetime, ab_testing_session, ab_testing_record, COUNT(ab_testing_session) AS ab_testing_count FROM ab_testing WHERE(ab_testing_userid = '" + _userID + "') GROUP BY ab_testing_session ORDER BY ab_testing_session";
		...
		private function buildDiagram(event:Object):void
		{
			...
			for (var index:int = 0; index < _testingAB.length; index++)
			{
				...
				_diagram.graphics.beginFill(0xFF0000);
				X = 100 + (step * count);
				Y = 250 - (length < 4 ? 50 * int(_testingAB[index]["count"]) : (50 / Math.ceil(length / 4)) * int(_testingAB[index]["count"]));
				_diagram.graphics.drawCircle(X, Y, 5);
				addChild(_diagram);
				
				_label = new Label(); // количество 
				_label.text = _testingAB[index]["record"]; 
				_label.x = 105 + (step * count);
				_label.y = Y - 10;
				_label.width = 50;
				_label.height = 50;
				addChild(_label);
