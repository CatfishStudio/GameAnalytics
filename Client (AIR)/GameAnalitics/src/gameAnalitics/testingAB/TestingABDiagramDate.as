package gameAnalitics.testingAB 
{
	 ...
	 */
	public class TestingABDiagramDate extends Sprite 
	{
		...
		public function TestingABDiagramDate(month:String, year:String) 
		{
			super();
		...
		{
			var sqlCommand:String = "SELECT * FROM ab_testing WHERE ab_testing_datetime BETWEEN '" + _year + "-" + _month + "-" + "01' AND '" + _year + "-" + _month + "-" + "30' ORDER BY ab_testing_userid";
		...
		private function buildTable(event:Object):void
		{
			...
			for (var i:Object in json_data) 
			{
				for (var k:Object in json_data[i].testing_ab) 
				{
					...
					if (lastUser == "")
					{
						lastUser = json_data[i].testing_ab[k].ab_testing_userid;
						userCount++;
					}
					else {
						if (lastUser != json_data[i].testing_ab[k].ab_testing_userid)
						{
							lastUser = json_data[i].testing_ab[k].ab_testing_userid;
							userCount++;
						}
			...
			_dataGrid.columns = ["user" , "data", "session", "record"];
			...
			_dataGrid.columns[2].headerText = "Сессия";
			_dataGrid.columns[2].width = 50;
			_dataGrid.columns[3].headerText = "Запись";
		...
		{
			var sqlCommand:String = "SELECT ab_testing_userid, ab_testing_datetime, ab_testing_session, ab_testing_record, COUNT(ab_testing_session) AS ab_testing_count FROM ab_testing WHERE ab_testing_datetime BETWEEN '" + _year + "-" + _month + "-" + "01' AND '" + _year + "-" + _month + "-" + "30' GROUP BY ab_testing_session  ORDER BY ab_testing_session";
		...
		private function buildDiagram(event:Object):void
		{
			...
			for (var index:int = 0; index < _testingAB.length; index++)
			{
				...
				_label.text = _testingAB[index]["record"]; 
