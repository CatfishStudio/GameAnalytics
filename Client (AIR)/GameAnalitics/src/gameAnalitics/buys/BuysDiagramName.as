package gameAnalitics.buys 
{
	 ...
	 */
	public class BuysDiagramName extends Sprite 
	{
		...
		public function BuysDiagramName(objectName:String) 
		{
			super();
		...
		{
			var sqlCommand:String = "SELECT * FROM buys WHERE (buys_object_name = '" + _objectName + "') ORDER BY buys_datetime";
		...
		private function buildTable(event:Object):void
		{
			...
			_dataGrid.columns = ["data", "user", "num", "price", "sum", "view", "location"];
		...
		{
			var sqlCommand:String = "SELECT buys_userid, buys_datetime, buys_object_name, buys_view, buys_location, buys_price, SUM(buys_number) AS buys_number, SUM(buys_sum) AS buys_sum FROM buys WHERE(buys_object_name = '" + _objectName + "') GROUP BY buys_location ORDER BY buys_location";
		...
		private function buildDiagram(event:Object):void
		{
			...
			for (var index:int = 0; index < _buys.length; index++)
			{
				/* Наименование */
				_label = new Label(); 
				_label.text = _buys[index]["location"]; 
